...
// FromJsonString It is highly **NOT** recommended to use this function
// because it has no param check, nor strict type check
func (r *DescribeModelQuotaRequest) FromJsonString(s string) error {
	f := make(map[string]interface{})
	if err := json.Unmarshal([]byte(s), &f); err != nil {
		return err
	}
...
type DescribeModelQuotaResponse struct {
	*tchttp.BaseResponse
	Response *DescribeModelQuotaResponseParams `json:"Response"`
}

func (r *DescribeModelQuotaResponse) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}
...
// FromJsonString It is highly **NOT** recommended to use this function
// because it has no param check, nor strict type check
func (r *DescribeModelQuotaResponse) FromJsonString(s string) error {
	return json.Unmarshal([]byte(s), &r)
}
...
type Model struct {
	...
	// <p>停止新购时间</p>
	DiscontinuedAt *string `json:"DiscontinuedAt,omitnil,omitempty" name:"DiscontinuedAt"`
}
